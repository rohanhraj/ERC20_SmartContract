...
describe("ERC20 smart contract testing", function () {
  ...
  it("You donot have permission to withdraw ether from contract", async function () {
    const wallet = token.connect(accounts[2]);
    await expect(wallet.withdraw(amount)).to.be.reverted;
  });

  it("Transfer amount to destination address", async function () {
  ...
  it("Cannot transfer above the amount", async function() {
    ...
    await expect(wallet.transfer(accounts[3].address, 2)).to.be.reverted;
  });

  it("Cannot transfer from empty account", async function () {
    const wallet = token.connect(accounts[3]);
    await expect(wallet.transfer(accounts[3].address, amount)).to.be.reverted;
  });
  ...
  it("Test to burning tokens", async function () {
  ...
  it("donot have enough ether to buy tokens",async function(){
    const wallet = token.connect(accounts[3]);
    const big_amount = ethers.utils.parseEther("9999");
    const option = {value:big_amount};
    await expect(wallet.buy(option)).to.be.reverted;
  //   let error;
  //   try{
  //     await wallet.buy(option);
  //   }catch(err){
  ...
  //   }
  //   expect(error).to.equal("Sender has Insufficient funds");
   });
});
